...
fn find_package(name: &str) -> Vec<PathBuf> {
    ...
    {
        // Try pkg-config but don't fail for optional packages like libyuv
        match std::panic::catch_unwind(|| {
            ...
            Err(_) => {
                if name == "libyuv" {
                    eprintln!("WARNING: libyuv not found via pkg-config, skipping...");
                    vec![]
                } else {
...
fn generate_bindings(
    ...
    if include_paths.is_empty() {
        eprintln!("WARNING: No include paths available, skipping binding generation for {}", ffi_header.display());
        return;
    }
...
fn gen_vcpkg_package(package: &str, ffi_header: &str, generated: &str, regex: &str) {
    let mut includes = find_package(package);
    
    // Linux fallback for libyuv when pkg-config does not return include paths.
    if includes.is_empty() && package == "libyuv" && cfg!(target_os = "linux") {
        let local_include = PathBuf::from("/usr/local/include");
        let system_include = PathBuf::from("/usr/include");
        if local_include.exists() {
            includes.push(local_include);
        } else if system_include.exists() {
            includes.push(system_include);
        }
    }
    
    // Skip if includes is empty (package not found)
    if includes.is_empty() {
        eprintln!("WARNING: {} not found, skipping generation of {}", package, generated);
        return;
    }
